...
function ChartContent({ data, itemHeight, faculty }: { data: { name: string; count: number; avgPCS: number; avgMCS: number }[]; itemHeight: number; faculty: SF12Response[] }) {
  return (
    <>
  ...
  if (faculty.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6">
        <h3 className="font-semibold text-gray-900 mb-1">Teaching Load (Previous Semester)</h3>
    ...
    <>
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6">
        <div className="flex items-center justify-between mb-1">
          <h3 className="font-semibold text-gray-900">Teaching Load (Previous Semester)</h3>
          <button
            type="button"
        ...
        <ChartContent data={data} itemHeight={55} faculty={faculty} />
      </div>

      {expanded && (
        <ChartModal title="Teaching Load (Previous Semester)" onClose={() => setExpanded(false)}>
          <ChartContent data={data} itemHeight={70} faculty={faculty} />
        </ChartModal>
      )}
    </>
  );
