...
function ChartContent({
  ...
  yes: { count: number; avgPCS: number; avgMCS: number };
  no: { count: number; avgPCS: number; avgMCS: number };
  total: number;
  pieHeight: number;
}) {
  return (
          ...
          <tbody>
            {[
              ...
              <tr key={label} className="border-b border-gray-100">
                ...
                <td className="py-2 pr-4">{stats.count > 0 ? stats.avgPCS.toFixed(2) : "—"}</td>
                <td className="py-2 pr-4">{stats.count > 0 ? stats.avgMCS.toFixed(2) : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function WalkableSpacesChart({ responses }: WalkableSpacesChartProps) {
  ...
  function insight(): string {
    if (total === 0 || yes.count === 0 || no.count === 0) return "";
    ...
    <>
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6">
        <div className="flex items-center justify-between mb-1">
          <h3 className="font-semibold text-gray-900">Walkable Spaces Availability</h3>
          {total > 0 && (
            <button
              type="button"
        ...
        {total === 0 ? (
          ...
          <>
            {insight() && <p className="text-sm text-gray-600 mb-4">{insight()}</p>}
            <ChartContent pieData={pieData} yes={yes} no={no} total={total} pieHeight={220} />
          </>
        )}
      </div>

      {expanded && (
        <ChartModal title="Walkable Spaces Availability" onClose={() => setExpanded(false)}>
          {insight() && <p className="text-sm text-gray-600 mb-4">{insight()}</p>}
          <ChartContent pieData={pieData} yes={yes} no={no} total={total} pieHeight={360} />
        </ChartModal>
      )}
    </>
  );
